...
ENV \
  ...
  TERM=xterm 
RUN \
  apk add --update --no-cache gcompat libstdc++ curl tzdata nano ca-certificates wget fuse3 python3 build-base py3-pip python3-dev py3-dotenv py3-schedule py3-psutil && ln -sf python3 /usr/bin/python && \
